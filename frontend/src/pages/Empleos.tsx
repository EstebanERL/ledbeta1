...
import { Loader2, Search, MapPin, Building2, Briefcase, Sparkles, ArrowLeft } from "lucide-react";
...
export default function EmpleosPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data } = await api.get("/vacantes/public", { params });
      setItems(data.items);
    } finally { setLoading(false); }
  };

  useEffect(() => { load(); /* eslint-disable-next-line */ }, [dept, mod, contrato]);
              ...
              <article key={v.id} className="group flex flex-col rounded-2xl border bg-card p-6 shadow-sm transition hover:shadow-elegant hover:-translate-y-0.5">
                ...
                <div className="mt-auto pt-4">
                  <Link to="/auth?tab=register">
                    <Button className="w-full bg-gradient-primary"><Briefcase className="mr-2 h-4 w-4" />Postularme</Button>
                  </Link>
                </div>
              </article>
            ))}
